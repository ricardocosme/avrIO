...
This library can be useful in the application layer delivering an expressive code(see [[file:demo/led.cpp][demo/led.cpp]]) but it can be also used to develop flexible and concise APIs(see [[file:demo/c++20/api.cpp][demo/c++20/api.cpp]]).
